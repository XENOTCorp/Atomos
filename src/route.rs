...
impl Router {
    pub fn dispatch(&self, mut req: In<'_>) -> Out {
        if self.gov.hard_block() {
            return self.err_out(ServeError::Capacity, "resource bound");
        }
        ...
        if let Some(pre) = &self.pre {
            match pre.handle(&req) {
                Ok(out) if out.status.as_u16() >= 400 => return out,
                Ok(out) => {
                    req.flags = out.flags;
                ...
                Err(e) => return self.err_out(e, "pre"),
            }
        }
        let rules = self.rules.load();
        let Some(rule) = rules.match_path(req.method.as_str(), req.path) else {
            return self.err_out(ServeError::NoRule, "no rule");
        };
        if let Some(hr) = header_fail(rule, &req) {
            return self.err_out(hr, "header rule");
        }
        let Some(handler) = self.modules.get(&rule.module) else {
            return self.err_out(
                ServeError::Module(rule.module.clone().into_boxed_str()),
                "missing module",
            );
        };
        let mut out = match handler {
            Handler::Sync(module) => match module.handle(&req) {
                Ok(o) => o,
                Err(e) => return self.err_out(e, "module"),
            },
            Handler::Async(_) => {
                return self.err_out(
        ...
        if let Some(post) = &self.post {
            if !req.flags.contains(FLAG_NO_POST) && !out.flags.contains(FLAG_NO_POST) {
                match post.handle(&req) {
                    ...
                    Err(e) => return self.err_out(e, "post"),
                }
            }
        }
