...
async fn main() {
    ...
    {
        eprintln!(
            "atomos-proto [--config FILE] [--bind HOST:PORT] [--root DIR] [--rules FILE]\nDefault bind 127.0.0.1:8090. engine=tokio http2/http3 on."
        );
        std::process::exit(0);
    }
    ...
    let mut cfg = if cfg_path.exists() {
        ...
        let mut c = Config::from_json(
            br#"{"bind":"127.0.0.1:8090","engine":"tokio","http2":true,"http3":true}"#,
        )
        .expect("default config");
        c.apply_host_file();
        c
    };
    cfg.engine = "tokio".into();
    if !cfg.http2 && !cfg.http3 {
        cfg.http2 = true;
        cfg.http3 = true;
    }
    ...
    let kind = EngineKind::parse(&cfg.engine).unwrap_or(EngineKind::Tokio);
    if kind != EngineKind::Tokio {
        eprintln!("atomos-proto requires engine=tokio");
        std::process::exit(1);
    }
